...
RUN FILTER=$(./android list sdk --all | grep -E -v "[0-9]{1,3}.*\(Obsolete\)$" | grep -E -v "[0-9]{1,3}.*[Dd]ocumentation.*" | grep -E -v "[0-9]{1,3}.*[Ss]ystem [Ii]mage.*" | grep -E -v "[0-9]{1,3}.*[Gg]lass [Dd]evelopment [Kk]it.*" | grep -E -v "[0-9]{1,3}.*[Ss]ources.*" | grep -E -v "[0-9]{1,3}.*[Ee]mulator.*" | grep -E -v "[0-9]{1,3}.*[Ss]imulators.*" | grep -E -v "[0-9]{1,3}.*[Ww]eb [Dd]river.*" | grep -E "^ +[0-9]{1,3}" | cut -d "-" -f 1 | xargs echo | sed -e "s/ /,/g") && echo y | android update sdk --no-ui --all --filter $FILTER
